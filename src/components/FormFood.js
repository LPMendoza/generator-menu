      ...
      <div className="contCard shadow-sm bg-white p-4 rounded formFood">
         ...
         <form className="mt-4">
            <div className="form-group">
               ...
               <textarea  rows="1" onChange={(e) => {resizeOnChange(e); handleOnInput(e);}} value={form.name} type="text" name="name" id="txtName" className={`form-control custom-input ${form.name == "" ? "invalid" : ""}`} placeholder="Ejem. Platano, Huevo, ...." autoFocus/>
            </div>
            <div className="form-group">
            ...
            <div className="mt-4">
               <button type="submit" onClick={form.name != "" ? handleOnSubmit : null} className={`btn btn-primary shadow-sm ${form.name == "" ? "disabled" : ""} ${isEditing ? "" : "btn-block"}`}>Guardar <span className="fas fa-save ml-2"></span></button>
